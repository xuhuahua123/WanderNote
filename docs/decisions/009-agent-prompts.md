...
Photo-Agent 已由 opencode + MiMo-V2.5 Pro 先行实现并提交。

下一步优先派发后端 Agent，不要一次性要求完成全部后端功能。

后端 Agent 第一阶段目标：

```text
优先完成 M0、M1、M2，目标是让已提交的 Photo-Agent 能够联调。
暂不实现 AI 写作和完整前端路线业务。
```
...
当前优先级：
Photo-Agent 代码已经先行提交。请优先完成 M0、M1、M2，目标是让 Photo-Agent 能够和后端联调。
