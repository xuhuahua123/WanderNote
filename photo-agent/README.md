...
```cmd
# Windows CMD
set AGENT_ENV=dev   # 开发环境
set AGENT_ENV=prod  # 生产环境
python -m agent.main
```
...
|------|------|------|
| 目录扫描 | ✅ 通过 | 成功发现 8 张照片 |
| EXIF 提取 | ✅ 通过 | 提取拍摄时间、GPS、尺寸 |
| thumb 生成 | ✅ 通过 | WebP 格式，长边 360px |
| preview 生成 | ✅ 通过 | WebP 格式，长边 1280px |
| SQLite 操作 | ✅ 通过 | 索引存储和状态跟踪 |
| 多环境配置 | ✅ 通过 | .env.dev/.env.prod 切换 |
| 优雅关闭 | ✅ 通过 | SIGINT/SIGTERM 处理 |
